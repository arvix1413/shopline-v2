    ...
    <section className="py-24" style={{ backgroundColor: '#F5F4FF', color: '#0A0A1A' }}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-4">
          <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#1E40AF' }}>{t.oneStop.eyebrow}</p>
          <h2 className="font-black mb-4" style={{ color: '#0A0A1A', fontSize: 40 }}>{t.oneStop.title}</h2>
          <p className="max-w-2xl mx-auto leading-relaxed text-sm" style={{ color: '#4B4B6B' }}>{t.oneStop.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-12">
          {t.oneStop.items.map((item, i) => (
            <Link key={i} href={hrefs[i]}
              ...
              style={{ backgroundColor: '#fff', border: '1px solid rgba(30,64,175,0.12)' }}>
              ...
              <h3 className="font-black text-sm mb-1" style={{ color: '#0A0A1A' }}>{item.name}</h3>

          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            {t.growth.items.map((item, i) => (
              <a key={i} href={growthHrefs[i]}
                ...
                style={{ border: '1px solid rgba(91,95,240,0.12)', backgroundColor: '#fff' }}>
                ...
                <div className="p-8">
                  <h3 className="text-xl font-black mb-3" style={{ color: '#12131F' }}>{item.title}</h3>
                  <p className="leading-relaxed mb-5 text-sm" style={{ color: '#5C5F7A' }}>{item.desc}</p>
                  <span className="font-bold text-sm" style={{ color: '#5B5FF0' }}>{t.growth.learnMore} →</span>
                </div>
              </a>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {t.growth.supports.map((s, i) => (
              <div key={i} className="rounded-2xl p-6 transition-all hover:shadow-md"
                style={{ border: '1px solid rgba(91,95,240,0.1)', backgroundColor: '#fff' }}>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 text-xl"
                  style={{ background: 'linear-gradient(135deg, rgba(91,95,240,0.14), rgba(72,76,232,0.12))' }}>
                  {s.icon}
                </div>
                <h4 className="font-black mb-2 text-sm" style={{ color: '#12131F' }}>{s.title}</h4>
            ...
            <div className="flex justify-center gap-2">
              {t.growth.testimonials.map((_, i) => (
                <button key={i} onClick={() => setActiveTestimonial(i)}
                  ...
                  style={{ width: i === activeTestimonial ? 24 : 8, backgroundColor: i === activeTestimonial ? '#5B5FF0' : 'rgba(18,19,31,0.15)' }}
                  aria-label={`${i + 1}`} />
              ))}
            </div>
          </div>
